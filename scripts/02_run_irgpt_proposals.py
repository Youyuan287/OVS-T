#!/usr/bin/env python3
"""Create IRGPT proposal JSONL for dataset-v2.

The script is intentionally command-template based because the local IRGPT repo
has several experimental entrypoints. If --irgpt_command is omitted, it emits
auditable low-confidence text proposals from the prompt bank so downstream SAM3
and QC stages can be tested without pretending IRGPT succeeded.
"""
...
def fallback_candidates(classes: List[Dict[str, Any]], max_classes: int) -> List[Dict[str, Any]]:
    ...
    for item in picked:
        ...
        out.append({
            ...
            "reason": "fallback_prompt_bank_candidate_without_irgpt_box",
        })
    return out
...
def main() -> int:
    ...
    parser.add_argument("--out_jsonl", default="/home/Groups/group2/Working/TJY/sam3_ir_test/outputs/dataset_v2_prompt_irgpt_sam3_qwen8b/irgpt_proposals.jsonl")
    parser.add_argument("--summary", default="")
    parser.add_argument("--max_images", type=int, default=500)
    parser.add_argument("--seed", type=int, default=33)
    parser.add_argument("--fallback_classes", type=int, default=12)
    parser.add_argument("--irgpt_command", default="", help="Command template with {image} and {prompt_bank}; stdout should be JSON.")
    ...
    stats = {"images": 0, "candidates": 0, "used_external_irgpt": bool(args.irgpt_command), "fallback_images": 0, "empty_or_error_raw": 0}

    with out_path.open("w", encoding="utf-8") as f:
        for idx, image in enumerate(images):
            raw_text = ""
            raw_json = None
            if args.irgpt_command:
                raw_text = run_command(args.irgpt_command, image, prompt_bank_path, args.timeout)
    ...
    summary_path = Path(args.summary) if args.summary else out_path.with_name("irgpt_proposals_summary.json")
