...
def set_stage2_trainable(model):
    """
    Stage2：
    - 继续冻结 MobileCLIP/text/language 相关模块，保护开放词汇能力；
    - 解冻视觉 backbone + decoder/head，让模型进一步适配红外域；
    - 注意模型仍保持 eval()，避免 EfficientSAM3 内部 matching 分支。
    """
    freeze_keywords = [
        "text",
        "clip",
...
def encode_image_once(model, processor, image_pil, device):
    ...
    # Stage2 解冻视觉分支，因此这里不能 no_grad；
    # 但同一张图仍然只编码一次，供多个 prompt 复用。
...
def main():
    ...
    parser.add_argument("--init_stage1_ckpt", required=True, help="Stage1 best checkpoint, e.g. best_by_iou.pth")
    ...
    print(f"[Info] init stage1 ckpt={args.init_stage1_ckpt}", flush=True)
    stage1_ckpt = torch.load(args.init_stage1_ckpt, map_location=device)
    missing, unexpected = model.load_state_dict(stage1_ckpt["model"], strict=False)
    print(f"[Info] loaded Stage1 checkpoint. missing={len(missing)}, unexpected={len(unexpected)}", flush=True)
