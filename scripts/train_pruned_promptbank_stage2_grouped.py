...
def set_stage1_trainable(model):
    """
    Pruned Stage2:
    - 继续冻结 vision_backbone 和 prompt_bank；
    - 解冻 transformer / geometry_encoder / segmentation_head / dot_prod_scoring；
    - 学习率更小，用于在 Stage1 fast 后进一步适配。
    """
    train_keywords = [
        "transformer",
        "geometry_encoder",
...
def main():
    ...
    parser.add_argument("--init_ckpt", required=True, help="Pruned Stage1 checkpoint, e.g. best_by_iou.pth")
    ...
    print(f"[Info] load pruned base: missing={len(missing)}, unexpected={len(unexpected)}", flush=True)

    init_ckpt = torch.load(args.init_ckpt, map_location=device)
    init_sd = init_ckpt["model"] if isinstance(init_ckpt, dict) and "model" in init_ckpt else init_ckpt
    missing2, unexpected2 = model.load_state_dict(init_sd, strict=False)
    print(f"[Info] load Stage1 init: missing={len(missing2)}, unexpected={len(unexpected2)}", flush=True)
